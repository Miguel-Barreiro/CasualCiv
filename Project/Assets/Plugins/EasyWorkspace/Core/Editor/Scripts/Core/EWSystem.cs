...
namespace EasyWorkspace
{
    public static class EWSystem
    {
        ...
        {
            EditorWindow[] projectBrowsers = Resources.FindObjectsOfTypeAll(Type.GetType("UnityEditor.ProjectBrowser,UnityEditor")) as EditorWindow[];
            EditorWindow[] activeProjectBrowsers = projectBrowsers.Where(window => !window.GetPropertyValue<bool>("isLocked") && window.hasFocus).ToArray();
            int numberOfExpandedProjectBrowsers = activeProjectBrowsers.Count(window => window.GetFieldValue<int>("m_ViewMode") == 1);
            foreach (EditorWindow projectBrowser in activeProjectBrowsers)
            {
                if (projectBrowser.GetFieldValue<int>("m_ViewMode") == 1)
                {
                    projectBrowser.InvokeMethod("SetFolderSelection", new[] { instanceId }, false);
                }
                else if (numberOfExpandedProjectBrowsers == 0)
                {
                    Selection.activeObject = EditorUtility.InstanceIDToObject(instanceId);

                    projectBrowser.GetFieldValue("m_AssetTree")?.GetPropertyValue("data")?.InvokeMethod("SetExpanded", instanceId, true);
                }
            }
        }
        ...
        public static void StartDrag(EWIAssetContainer assetContainer)
        {
            ...
            DragAndDrop.paths = new[] { AssetDatabase.GetAssetPath(assetContainer.Asset) };
